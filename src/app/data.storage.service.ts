...
export class DataStorageService {
  ...
  getUgovori(u: FormGroup): Observable<Ugovor[]> {
    let querryParams = u.value['imeprezime'] ? 'imeprezime=' + u.value['imeprezime'] + '&' : '';
    querryParams += u.value['lbroj'] ? 'lbroj=' + u.value['lbroj'] + '&' : '';
    querryParams += u.value['maticnievidencija'] ? 'maticnievidencija=' + u.value['maticnievidencija'] + '&' : '';
    querryParams += u.value['radnomesto'] ? 'radnomesto=' + u.value['radnomesto'] + '&' : ''
    querryParams += u.value['datumzasnivanja'] ?'datumzasnivanja=' + [u.value['datumzasnivanja'].from,u.value['datumzasnivanja'].to] + '&' : ''
    querryParams += u.value['netozarada'] ? 'netozarada=' + u.value['netozarada'] + '&' : ''
    querryParams += u.value['brutozarada'] ? 'brutozarada=' + u.value['brutozarada'] + '&' : ''
    querryParams += u.value['datumaktazarade'] ? 'datumaktazarade=' + u.value['datumaktazarade'] + '&' : ''
    querryParams += u.value['datumprestanka'] ? 'datumprestanka=' +  [u.value['datumprestanka'].from,u.value['datumprestanka'].to] + '&' : ''
    querryParams += u.value['mestorada'] ? 'mestorada=' + u.value['mestorada'] + '&' : ''
    querryParams += u.value['prijavljenastrucnasprema'] ? 'prijavljenastrucnasprema=' + u.value['prijavljenastrucnasprema'] + '&' : ''
    querryParams += u.value['zahtevnastrucnasprema'] ? 'zahtevnastrucnasprema=' + u.value['zahtevnastrucnasprema'] + '&' : ''
    querryParams += u.value['pravnolice'] ? 'pravnolice=' + u.value['pravnolice'] + '&' : ''
    if (querryParams.charAt(querryParams.length - 1) === '&') {
      querryParams = querryParams.substr(0, querryParams.length - 1)
    }
    return this.http.get<Ugovor[]>(this.ugovoriUrl + (querryParams ? '?' + querryParams : ''));
  }

  SearchRadnikItem(r: FormGroup): Observable<Radnik[]> {
    let querryParams = r.value['imeprezime'] ? 'imeprezime=' + r.value['imeprezime'] + '&' : '';
    querryParams += r.value['jmbg'] ? 'jmbg=' + r.value['jmbg'] + '&' : '';
    querryParams += r.value['adresa'] ? 'adresa=' + r.value['adresa'] + '&' : '';
    querryParams += r.value['mesto'] ? 'mesto=' + r.value['mesto'] + '&' : ''
    querryParams += r.value['telefon'] ? 'telefon=' + r.value['telefon'] + '&' : ''
    querryParams += r.value['mobilni'] ? 'mobilni=' + r.value['mobilni'] + '&' : ''
    querryParams += r.value['email'] ? 'email=' + r.value['email'] + '&' : ''
    querryParams += r.value['strucnasprema'] ? 'strucnasprema=' + r.value['strucnasprema'] + '&' : ''
    querryParams += r.value['stepenstrucnespreme'] ? 'stepenstrucnespreme=' + r.value['stepenstrucnespreme'] + '&' : ''
    querryParams += r.value['stanjeZaposlenogRadnika'] ? 'stanjeZaposlenogRadnika=' + r.value['stanjeZaposlenogRadnika'] + '&' : ''
    querryParams += r.value['napomena'] ? 'napomena=' + r.value['napomena'] + '&' : ''
    querryParams += r.value['aktivan'] ? 'aktivan=' + r.value['aktivan'] + '&' : ''


    if (querryParams.charAt(querryParams.length - 1) === '&') {
      querryParams = querryParams.substr(0, querryParams.length - 1)
    }
    return this.http.get<Radnik[]>(this.radniciUrl + '/search' + (querryParams ? '?' + querryParams : ''));
